/**
 * receipt-preview.tsx  (2026-07-09)
 * Route: /voucher/receipt-preview?tdkRef=TDK-RCP-2026-0001
 *
 * Mirrors sales/invoice-preview.tsx pattern:
 *   • Fetches receipt snapshot from backend (/tally/invoice/:tdkRef/preview)
 *     — the same endpoint works for receipts because buildVoucherDocument()
 *       now branches on voucher_type='receipt'.
 *   • Shows real Tally voucher number when synced, provisional watermark otherwise.
 *   • Auto-refreshes on invoice_posting_updated WebSocket event.
 */
...
export default function ReceiptPreviewScreen() {
  ...
  const fetchPreview = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const res: any = await getReceiptPreview(tdkRef, company.guid);
      if (res?.status && res?.data) {
        setDoc(res.data);
        setIsProvisional(res.data.isProvisional ?? false);
        setPostingTag(res.data.postingTag || 'Not Posted');
      } else {
        setError('Could not load receipt preview.');
      }
    } catch (e: any) {
  ...
  const handleShare = async () => {
    ...
    try {
      const lines = [
        `Receipt Voucher — ${doc.documentNumber}`,
        `TDK Ref: ${doc.tdkRef}`,
        `Date: ${fmtDate(doc.documentDate)}`,
        '',
        `From: ${doc.party?.name || '—'}`,
        `Amount: ${fmtINR(doc.totals?.grandTotal)}`,
        `Method: ${doc.receipt?.paymentMethod || 'Cash'}`,
        `Via: ${doc.receipt?.ledgerAccount || '—'}`,
        '',
        'Bill Allocations:',
        ...((doc.receipt?.billAllocations || []).map((b: any) =>
    ...
    return (
      <SafeAreaView style={s.safe}>
        <View style={s.hdr}>
          ...
          <Text style={s.hdrTitle}>Receipt Preview</Text>
        </View>
        <View style={s.center}>
          <ActivityIndicator size="large" color={COLORS.brandPrimary} />
          <Text style={s.loadingTxt}>Loading receipt...</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    return (
      <SafeAreaView style={s.safe}>
        ...
        <View style={s.center}>
          ...
          <Text style={s.errTxt}>{error || 'Receipt not found'}</Text>
  ...
  const bills = doc.receipt?.billAllocations || [];
  const instrument = doc.receipt?.instrument;
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.card}>
          ...
          <View style={s.row}>
            ...
            <Text style={s.cardValSm}>{doc.receipt?.paymentMethod || 'Cash'}</Text>
          </View>
          <View style={s.row}>
            <Text style={s.cardLbl}>Ledger</Text>
            <Text style={s.cardValSm}>{doc.receipt?.ledgerAccount || '—'}</Text>
          </View>
          {instrument && (
            <>
              <View style={s.cardDiv} />
      ...
      <View style={s.actionBar}>
        ...
        <TouchableOpacity style={[s.actionBtn, s.actionPrimary]} onPress={() => router.replace('/voucher/receipt' as any)} activeOpacity={0.85}>
