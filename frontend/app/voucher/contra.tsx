/**
 * Contra list — rewrite (2026-07-14), mirrors Journal list polish.
 */
...
export default function ContraVouchersScreen() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      setLiveItems(rows.map((r: any) => {
        ...
        const tdkRef =
          (r.narration || '').match(/TDK-(?:OPT-)?CON-\d{4}-\d+/i)?.[0]
          || (r.reference || '').match(/TDK-(?:OPT-)?CON-\d{4}-\d+/i)?.[0]
          || null;
        const narration = r.narration || '';
        const fromTo = narration.includes('→')
          ? narration
          : (r.party_name || '—');
        return {
      ...
      setApiError(err?.message || 'Failed to load contras');
    } finally {
      setIsLoading(false);
      setRefreshing(false);
    }
  ...
  const totalAmt = filtered.reduce((sum, i) => sum + (i.amount || 0), 0);
