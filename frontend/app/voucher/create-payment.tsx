...
export default function CreatePaymentVoucher() {
  ...
  const handleSubmit = () => {
    ...
    Alert.alert('✅ Success', `Payment Voucher ${voucherNo} submitted successfully!`, [{ text: 'OK', onPress: () => router.back() }]);
  };

  const handleSaveOptional = () => {
    Alert.alert('Saved', `Payment Voucher saved as optional (not posted to books).`, [{ text: 'OK', onPress: () => router.back() }]);
  };
