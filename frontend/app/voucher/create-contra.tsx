...
export default function CreateContraVoucher() {
  ...
  const handleSubmit = () => {
    ...
    Alert.alert('✅ Success', `Contra Voucher ${voucherNo} submitted successfully!`, [{ text: 'OK', onPress: () => router.back() }]);
  };

  const handleSaveOptional = () => {
    Alert.alert('Saved', `Contra Voucher saved as optional (not posted to books).`, [{ text: 'OK', onPress: () => router.back() }]);
  };
