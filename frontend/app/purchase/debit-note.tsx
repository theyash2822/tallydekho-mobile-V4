...
export default function DebitNotesScreen() {
  ...
  const filtered = liveData.filter(n => !search
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32 }}>
        ...
        <SearchBar value={search} onChangeText={setSearch} placeholder="Search debit notes..." />
        ...
        <View style={s.secHdr}><Text style={s.secT}>Debit Notes</Text></View>
        <View style={s.card}>
          {filtered.map((n, idx) => (
            <View key={n.id}>
              <TouchableOpacity style={s.row} activeOpacity={0.7}>
                <View style={s.rowL}>
                  ...
                  <View style={s.rInfo}>
                    ...
                    <Text style={s.party}>{n.vendor}</Text>
                    <Text style={s.meta}>{n.date}</Text>
                  </View>
                </View>
