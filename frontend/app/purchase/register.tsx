...
export default function PurchaseRegisterScreen() {
  ...
  const mapPurchaseInv = (r: any, i: number): PurchaseInvoice => ({
    ...
    vendor: r.party_name || r.primary_ledger || '',
    date: r.date || '',
    time: '',
      ...
      if (mode === 'combined') {
        ...
        const { shared, failed } = await shareVouchersAsMultiPagePdf(
          ...
          items.map(inv => ({
            ...
            documentType: docTypeToRouteType(inv.docType || 'invoice', 'purchase'),
            label: `${inv.voucherType || 'Purchase'}-${inv.number || inv.id}.pdf`,
          })),
          {
            fileName: `Purchase-Register (${items.length}).pdf`,
            onBeforeShare: () => setIsSharing(false),
          }
        );
        if (failed > 0) {
