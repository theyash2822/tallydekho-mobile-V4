/**
 * Stock Transfer preview — Tally Stock Journal voucher snapshot.
...
export default function StockTransferPreviewScreen() {
  ...
  const fetchPreview = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const res: any = await getStockTransferPreview(tdkRef, company.guid);
      if (res?.status && res?.data) {
        setDoc(res.data);
        setIsProvisional(res.data.isProvisional ?? false);
        setPostingTag(res.data.postingTag || 'Not Posted');
      } else {
        setError('Could not load transfer preview.');
      }
    } catch (e: any) {
  ...
  const handleShare = async () => {
    if (!doc) return;
    const tr = doc.stockTransfer || {};
    const lines = (tr.items || []).map((it: any) =>
      `  • ${it.itemName}: ${it.qty} ${it.unit || 'pcs'} (${it.fromGodown} → ${tr.toGodown})`,
    );
    try {
      await Share.share({
        message: [
          `Stock Journal — ${doc.documentNumber}`,
          `TDK Ref: ${doc.tdkRef}`,
          `Date: ${fmtDate(doc.documentDate)}`,
          `Destination: ${tr.toGodown || '—'}`,
          '',
          'Items:',
          ...lines,
          doc.narration ? `\nNote: ${doc.narration}` : '',
    ...
    return (
      <SafeAreaView style={s.safe}>
        <View style={s.hdr}>
          ...
          <Text style={s.hdrTitle}>Transfer Preview</Text>
        </View>
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.card}>
          <Text style={s.typeBadge}>{doc.tallyVoucherType || 'Stock Journal'}</Text>
        ...
        <View style={s.routeCard}>
          <View style={s.routeCol}>
            <Text style={s.cardLbl}>From</Text>
            <Text style={s.routeVal} numberOfLines={2}>
              {tr.fromGodown || (items.length > 1 ? 'Multiple' : items[0]?.fromGodown || '—')}
            </Text>
          </View>
          <Ionicons name="arrow-forward" size={18} color={COLORS.brandPrimary} />
          <View style={[s.routeCol, { alignItems: 'flex-end' }]}>
            <Text style={s.cardLbl}>To</Text>
            <Text style={s.routeVal} numberOfLines={2}>{tr.toGodown || '—'}</Text>
          </View>
        </View>

        <View style={s.card}>
          <Text style={s.secTitle}>Items ({items.length})</Text>
          {items.map((it: any, i: number) => (
            <View key={`${it.itemName}-${i}`} style={[s.itemRow, i > 0 && s.itemBorder]}>
              <View style={{ flex: 1 }}>
                <Text style={s.itemName} numberOfLines={1}>{it.itemName}</Text>
                <Text style={s.itemMeta}>
                  {it.fromGodown} → {tr.toGodown}
                </Text>
              </View>
              <Text style={s.itemQty}>{it.qty} {it.unit || 'pcs'}</Text>
            </View>
          ))}
        </View>
...
const s = StyleSheet.create({
  ...
  routeVal: { fontSize: TYPOGRAPHY.sm, fontWeight: '800', color: COLORS.textPrimary },
  ...
  itemQty: { fontSize: TYPOGRAPHY.sm, fontWeight: '800', color: COLORS.brandPrimary },
