...
interface TrItem { id: string; product: string; qty: string; unit: string; }
const newItem = (): TrItem => ({ id: Date.now().toString(), product: '', qty: '1', unit: 'Pcs' });
...
function TransferItemRow({ item, onUpdate, onRemove, onModal }: {
  item: TrItem; onUpdate:(id:string,f:keyof TrItem,v:string)=>void;
  ...
  return (
    <View style={ir.card}>
      ...
      <View style={ir.row}>
        <View style={ir.qBox}><Text style={ir.ml}>Qty to Transfer</Text>
          <TextInput style={ir.mi} value={item.qty} onChangeText={v=>onUpdate(item.id,'qty',v)} keyboardType="numeric" placeholder="0" placeholderTextColor={COLORS.textTertiary} />
        </View>
...
export default function CreateStockTransferScreen() {
  ...
  const [trNo] = useState('STR-0013');
  const [date, setDate] = useState(todayStr());
  const [fromWH, setFromWH] = useState('');
  const [toWH, setToWH] = useState('');
  const [transferDate, setTransferDate] = useState('');
  const [items, setItems] = useState<TrItem[]>([newItem()]);
  const [narration, setNarration] = useState('');
  const [activeModal, setActiveModal] = useState<ModalState>(null);

  const updateItem = useCallback((id:string,f:keyof TrItem,v:string)=>setItems(prev=>prev.map(i=>i.id===id?{...i,[f]:v}:i)),[]);
  const removeItem = useCallback((id:string)=>setItems(prev=>prev.length>1?prev.filter(i=>i.id!==id):prev),[]);

  const handleSubmit = useCallback(()=>{
    if (fromWH === toWH && fromWH !== '') {
      Alert.alert('Error', 'Source and destination warehouses cannot be the same.'); return;
    }
    Alert.alert('✓ Transfer Initiated', `Stock Transfer ${trNo} initiated successfully!`,[{text:'OK',onPress:()=>router.back()}]);
  },[trNo,router,fromWH,toWH]);

  const fromOptions = WAREHOUSES;
  const toOptions = WAREHOUSES.filter(w=>w.value!==fromWH);
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      <View style={s.header}>
        ...
        <View style={s.badge}><Text style={s.badgeTxt}>{trNo}</Text></View>
      </View>
      ...
      <KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==='ios'?'padding':undefined}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={s.card}>
            <View style={s.cardHdr}><Ionicons name="swap-horizontal-outline" size={18} color={COLORS.info} /><Text style={s.cardTitle}>Transfer Details</Text></View>
            <View style={s.row2}>
              <View style={{flex:1}}><Text style={s.fLabel}>Transfer No.</Text>
                <View style={s.autoBox}><Text style={s.autoTxt}>{trNo}</Text><Ionicons name="lock-closed-outline" size={13} color={COLORS.textTertiary} /></View>
              </View>
              <View style={{flex:1}}><Text style={s.fLabel}>Date <Text style={s.star}>*</Text></Text>
                <TextInput style={s.fInput} value={date} onChangeText={setDate} placeholder="DD/MM/YY" placeholderTextColor={COLORS.textTertiary} />
              </View>
            </View>

            <FormDropdown label="From Warehouse" value={fromWH} options={fromOptions} onSelect={o=>setFromWH(o.value)} placeholder="Select source..." required />

            <View style={s.arrowRow}>
              <View style={s.arrowLine} />
              <View style={s.arrowCircle}><Ionicons name="arrow-down" size={18} color={COLORS.info} /></View>
              <View style={s.arrowLine} />
            </View>

            <FormDropdown label="To Warehouse" value={toWH} options={toOptions} onSelect={o=>setToWH(o.value)} placeholder="Select destination..." required />

            <View style={{flex:1}}><Text style={s.fLabel}>Transfer Date</Text>
              <TextInput style={s.fInput} value={transferDate} onChangeText={setTransferDate} placeholder="DD/MM/YY (if different)" placeholderTextColor={COLORS.textTertiary} />
            </View>
          </View>
          ...
          {fromWH && toWH && (
            <View style={s.transferBanner}>
              ...
              <Text style={s.transferTxt}>
                <Text style={{fontWeight:'700'}}>{WAREHOUSES.find(w=>w.value===fromWH)?.label?.split(' - ')[0]}</Text>
                {' → '}
                <Text style={{fontWeight:'700'}}>{WAREHOUSES.find(w=>w.value===toWH)?.label?.split(' - ')[0]}</Text>
              </Text>
            </View>
          )}
          ...
          <View style={s.secHdr}>
            ...
            <Text style={s.secTitle}>Items to Transfer</Text>
            <View style={s.countBadge}><Text style={s.countTxt}>{items.length}</Text></View>
          </View>
          {items.map(item=>(
            <TransferItemRow key={item.id} item={item} onUpdate={updateItem} onRemove={removeItem} onModal={setActiveModal} />
          ))}
          ...
          <View style={s.card}>
            <FormField label="Narration" value={narration} onChangeText={setNarration} placeholder="Transfer reason / notes..." multiline numberOfLines={2}
        ...
        <View style={[s.footer,{paddingBottom:Math.max(insets.bottom,12)}]}>
          <TouchableOpacity style={s.submitBtn} onPress={handleSubmit} activeOpacity={0.7}>
            ...
            <Text style={s.submitTxt}>Initiate Transfer</Text>
          </TouchableOpacity>
        </View>
...
const s = StyleSheet.create({
  ...
  arrowRow:{flexDirection:'row',alignItems:'center',marginVertical:SPACING.xs,marginBottom:SPACING.md},
  arrowLine:{flex:1,height:1,backgroundColor:COLORS.borderDefault},
  arrowCircle:{width:36,height:36,borderRadius:18,backgroundColor:COLORS.infoBg,borderWidth:1,borderColor:COLORS.info+'40',alignItems:'center',justifyContent:'center',marginHorizontal:SPACING.md},
  transferBanner:{flexDirection:'row',alignItems:'center',gap:8,backgroundColor:COLORS.infoBg,borderRadius:RADIUS.md,padding:12,marginBottom:SPACING.md,borderWidth:1,borderColor:COLORS.info+'40'},
  transferTxt:{fontSize:TYPOGRAPHY.sm,color:COLORS.info,flex:1},
  ...
  submitBtn:{flexDirection:'row',gap:8,paddingVertical:14,borderRadius:RADIUS.md,backgroundColor:COLORS.info,alignItems:'center',justifyContent:'center'},
