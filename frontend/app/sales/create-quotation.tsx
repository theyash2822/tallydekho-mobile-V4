  ...
  return (
    <View style={ir.card}>
      ...
      <View style={ir.row}>
        ...
        <View style={ir.rBox}><Text style={ir.mLabel}>Rate (₹)</Text>
          <TextInput style={ir.mInput} value={item.rate} onChangeText={v=>onUpdate(item.id,'rate',v)} keyboardType="numeric" placeholder="0.00" placeholderTextColor={COLORS.textTertiary} />
        </View>
      </View>
      <View style={ir.row}>
...
export default function CreateQuotationScreen() {
  ...
  const updateItem = useCallback((id:string,f:keyof QItem,v:string)=>setItems(prev=>prev.map(i=>i.id===id?{...i,[f]:v}:i)),[]);
    ...
    <SafeAreaView style={s.safe} edges={['top']}>
      ...
      <KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==='ios'?'padding':undefined}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
          ...
          {/* Quotation Details */}
          <View style={s.card}>
            <View style={s.cardHdr}><Ionicons name="chatbubble-outline" size={18} color={COLORS.warning} /><Text style={s.cardTitle}>Quotation Details</Text></View>
            <View style={s.row2}>
              <View style={{flex:1}}>
                <Text style={s.fLabel}>Quotation No.</Text>
                <View style={s.autoBox}><Text style={s.autoTxt}>{qtNo}</Text><Ionicons name="lock-closed-outline" size={13} color={COLORS.textTertiary} /></View>
              </View>
              <View style={{flex:1}}>
          ...
          <View style={s.sumCard}>
            ...
            {totals.taxTotal>0&&<View style={s.sumRow}><Text style={s.sumL}>Total Tax (GST)</Text><Text style={s.sumV}>₹{totals.taxTotal.toLocaleString('en-IN',{minimumFractionDigits:2})}</Text></View>}
            <View style={s.sumDiv} />
            <View style={s.sumRow}><Text style={s.sumGrandL}>Quoted Amount</Text><Text style={s.sumGrandV}>₹{totals.grand.toLocaleString('en-IN',{minimumFractionDigits:2})}</Text></View>
          </View>

          {/* Notes */}
          <View style={s.card}>
            <View style={s.cardHdr}><Ionicons name="document-outline" size={18} color={COLORS.textSecondary} /><Text style={s.cardTitle}>Notes & Terms</Text></View>
      ...
      <Modal visible={activeModal?.type==='product'} transparent animationType="slide" onRequestClose={()=>setActiveModal(null)}>
        ...
        <View style={m.sheet}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            {PRODUCTS.map(p=>(
              <TouchableOpacity key={p.value} style={[m.opt,activeModal&&items.find(i=>i.id===activeModal.itemId)?.product===p.value&&m.optA]}
                onPress={()=>{if(activeModal)updateItem(activeModal.itemId,'product',p.value);setActiveModal(null);}} activeOpacity={0.7}>
