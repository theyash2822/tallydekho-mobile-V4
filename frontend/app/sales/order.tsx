  ...
  return (
    <SafeAreaView style={s.safe}>
      <View style={s.hdr}>
        ...
        <Text style={s.hdrTitle}>Sales Orders</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32 }}>
        ...
        <View style={s.search}>
          ...
          <TextInput style={s.searchIn} placeholder="Search orders, parties..." placeholderTextColor={COLORS.textTertiary} value={search} onChangeText={setSearch} />
        ...
        <View style={s.secHdr}><Text style={s.secT}>Sales Orders</Text><TouchableOpacity style={s.menuB}><Ionicons name="ellipsis-horizontal" size={20} color={COLORS.textSecondary} /></TouchableOpacity></View>
        <View style={s.card}>
          {filtered.length === 0 ? <View style={s.empty}><Ionicons name="search-outline" size={32} color={COLORS.textTertiary} /><Text style={s.emptyT}>No orders found</Text></View>
            : filtered.map((o, idx) => (
              <View key={o.id}>
                <TouchableOpacity style={s.row} activeOpacity={0.7}>
                  <View style={s.rowL}>
                    ...
                    <View style={s.rInfo}>
                      <View style={s.topR}><Text style={[s.stLbl, { color: SC[o.status] || '#9CA3AF' }]}>{SL[o.status]}</Text><Text style={s.docId}>{o.id}</Text></View>
                      <Text style={s.party}>{o.party}</Text>
                      <Text style={s.meta}>{o.date} · {o.time}</Text>
                    </View>
                  </View>
                  <View style={s.rowR}><Text style={s.amt}>{o.amount}</Text>
                    <TouchableOpacity style={s.shareB} activeOpacity={0.7}><Ionicons name="share-outline" size={14} color={COLORS.positive} /><Text style={s.shareT}>Share</Text></TouchableOpacity>
                  </View>
                </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  secHdr:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginHorizontal:SPACING.md,marginTop:SPACING.md,marginBottom:SPACING.sm}, secT:{fontSize:TYPOGRAPHY.base,fontWeight:'700',color:COLORS.textPrimary}, menuB:{width:32,height:32,alignItems:'center',justifyContent:'center'},
  ...
  div:{height:1,backgroundColor:COLORS.borderDefault,marginLeft:16}, empty:{alignItems:'center',paddingVertical:40,gap:8}, emptyT:{fontSize:TYPOGRAPHY.sm,color:COLORS.textTertiary},
});
