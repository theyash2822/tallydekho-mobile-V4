...
const BARCODE_MAP: Record<string, string> = {
  ...
  '345678901234': 'lycan_hp',   '456789012345': 'sony_xm5',
...
function BarcodeScannerModal({ visible, onScan, onClose }: {
  visible: boolean; onScan: (v: string) => void; onClose: () => void;
}) {
...
function ItemRow({ item, onUpdate, onRemove, onModal }: {
  item: OItem;
  onUpdate: (id:string, f:keyof OItem, v:string) => void;
...
// ─── DateInput Helper ───────────────────────────────────────────────────
function DateInput({ label, value, onChange, required, title }: {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <TouchableOpacity style={s.dateBox} onPress={() => setShowPicker(true)} activeOpacity={0.7}>
        <Text style={[s.dateTxt, !value && s.datePlh]}>{value || 'DD/MM/YY'}</Text>
        <Ionicons name="calendar-outline" size={18} color={COLORS.brandPrimary} />
      </TouchableOpacity>
      <DatePickerModal visible={showPicker} value={value} onSelect={onChange} onClose={() => setShowPicker(false)} title={title || label} />
    </View>
  );
}
...
export default function CreateSalesOrderScreen() {
  ...
  const updateItem = useCallback((id:string, f:keyof OItem, v:string) =>
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>Create Sales Order</Text>
        <RegularOptionalToggle value={entryType} onChange={setEntryType} />
        <View style={s.badge}><Text style={s.badgeTxt}>{orderNo}</Text></View>
      </View>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS==='ios' ? 'padding' : undefined}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={s.card}>
            ...
            <View style={s.row2}>
              <View style={{ flex: 1 }}>
                ...
                <View style={s.autoBox}><Text style={s.autoTxt}>{orderNo}</Text><Ionicons name="lock-closed-outline" size={13} color={COLORS.textTertiary} /></View>
              </View>
              <DateInput label="Date" required value={date} onChange={setDate} title="Order Date" />
            </View>
            <SearchableDropdown
          ...
          <View style={s.sumCard}>
            ...
            <View style={s.sumDiv} />
            <View style={s.sumRow}><Text style={s.sumGL}>Order Total</Text><Text style={s.sumGV}>₹{totals.grand.toLocaleString('en-IN',{minimumFractionDigits:2})}</Text></View>
          </View>

          <View style={s.card}>
            <FormField label="Narration" value={narration} onChangeText={setNarration} placeholder="Internal notes..." multiline numberOfLines={2}
          ...
          {(() => {
            ...
            return (
              <ScrollView showsVerticalScrollIndicator={false}>
                {item?.warehouse ? (
                  <View style={m.whHint}><Ionicons name="business-outline" size={13} color={COLORS.info} />
                    <Text style={m.whHintTxt}>Products from: {WAREHOUSES.find(w=>w.value===item.warehouse)?.label}</Text></View>
                ) : (
                  <View style={m.whHint}><Ionicons name="alert-circle-outline" size={13} color={COLORS.warning} />
                    <Text style={[m.whHintTxt,{color:COLORS.warning}]}>Select warehouse to filter products</Text></View>
                )}
                {filteredProducts.map(p => (
                  <TouchableOpacity key={p.value} style={[m.opt,item?.product===p.value&&m.optA]}
                    ...
                    <View style={m.optRow}><Ionicons name="cube-outline" size={16} color={COLORS.textSecondary} />
                      <Text style={[m.optTxt,item?.product===p.value&&m.optTxtA]}>{p.label}</Text></View>
      ...
      <BarcodeScannerModal
        ...
        onScan={(productValue) => {
          if(activeModal) {
            ...
            Alert.alert('✓ Product Found',`Added: ${ALL_PRODUCTS.find(p=>p.value===productValue)?.label||productValue}`);
          }
          closeModal();
        }}
        onClose={closeModal}
