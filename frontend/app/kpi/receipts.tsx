...
// ─── Chart constants ─────────────────────────────────────────────────────────
const CHART_COLOR = '#2D7D46';    // positive green for receipts (inflow)
...
// ─── Mock data ────────────────────────────────────────────────────────────────
const KPI_DATA = [
  { id: 'today', icon: 'calendar-outline',    label: 'Today',  amount: '₹1,34,000', trend: '+12%',  positive: true },
  { id: 'mtd',   icon: 'calendar-outline',    label: 'MTD',    amount: '₹45,000',   trend: '+10%',  positive: true },
  { id: 'ytd',   icon: 'stats-chart-outline', label: 'YTD',    amount: '₹65,000',   trend: '+18%',  positive: true },
  { id: 'cash',  icon: 'cash-outline',         label: 'Cash',   amount: '₹55,000',   trend: '+8%',   positive: true },
  { id: 'bank',  icon: 'business-outline',     label: 'Bank',   amount: '₹1,20,000', trend: '+16%',  positive: true },
];
...
const RECENT_RCP = [
  { id: 'r1', ref: 'RCP-2010', party: 'Sunrise Exports',  date: '10 Jul', mode: 'Bank', amount: '₹2,50,000', verified: true  },
  { id: 'r2', ref: 'RCP-2011', party: 'Metro Traders',    date: '08 Jul', mode: 'Cash', amount: '₹55,000',   verified: true  },
  { id: 'r3', ref: 'RCP-2012', party: 'Apex Industries',  date: '07 Jul', mode: 'Bank', amount: '₹1,80,000', verified: false },
  { id: 'r4', ref: 'RCP-2013', party: 'City Merchants',   date: '06 Jul', mode: 'Cash', amount: '₹32,000',   verified: true  },
];
...
function DailyChart() {
  ...
  const pan = useRef(PanResponder.create({
    ...
    onPanResponderRelease: () => { hideTimer.current = setTimeout(() => setActiveIdx(null), 3000); },
  })).current;
    ...
    <View style={ch.card}>
      ...
      <View {...pan.panHandlers}>
        <Svg width={CHART_W_FULL} height={CHART_H_SVG}>
          <Defs>
            <SvgGrad id="areaGradR" x1="0" y1="0" x2="0" y2="1">
          ...
          <PieChart
            ...
            centerLabelComponent={() => (
              <View style={dc.center}>
                <Text style={dc.centerAmt}>₹5.35L</Text>
        ...
        <View style={dc.legend}>
          {DONUT_DATA.map((item, i) => (
            ...
            >
              ...
              <Text style={dc.legendAmt}>₹{(item.value / 1000).toFixed(0)}K</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>
...
export default function ReceiptsScreen() {
  ...
  React.useEffect(() => {
    ...
    getKPIReceipts(companyGuid).then((res: any) => { if (res?.data) setApiData(res.data); }).catch(() => {}).finally(() => setIsLoading(false));
  }, [companyGuid]);
  ...
  const filtered = RECENT_RCP.filter(r => {
    if (typeTab === 'Cash') return r.mode === 'Cash';
    if (typeTab === 'Bank') return r.mode === 'Bank';
    return true;
  });
            ...
            >
              ...
              <View style={s.txInfo}>
                <View style={s.txTopRow}>
                  <Text style={s.txMode}>{r.mode}</Text>
                  <Text style={s.txRef}> · {r.ref}</Text>
                </View>
                <Text style={s.txSub}>{r.party} · {r.date}</Text>
              </View>
