...
const KPI_CARDS = [
  { label: 'Total Payable', value: '₹5,61,500', sub: '8 vendors', icon: 'people-outline', color: '#DC2626' },
  { label: 'Overdue', value: '₹2,10,000', sub: '3 vendors', icon: 'warning-outline', color: '#C0392B' },
  { label: 'Due This Week', value: '₹1,40,000', sub: '2 vendors', icon: 'calendar-outline', color: '#D97706' },
  { label: 'Paid MTD', value: '₹98,000', sub: 'This month', icon: 'checkmark-circle-outline', color: '#2D7D46' },
];

const OUTSTANDING = [
  { id: 'V1', name: 'ABC Suppliers', invoices: 4, amount: '₹1,42,000', daysOverdue: 0, due: '30 May' },
  { id: 'V2', name: 'Delhi Wholesale', invoices: 2, amount: '₹87,500', daysOverdue: 18, due: '07 May' },
  { id: 'V3', name: 'Tech Components Ltd', invoices: 1, amount: '₹2,10,000', daysOverdue: 32, due: '23 Apr' },
  { id: 'V4', name: 'National Traders', invoices: 3, amount: '₹68,000', daysOverdue: 0, due: '05 Jun' },
  { id: 'V5', name: 'Global Imports', invoices: 2, amount: '₹54,000', daysOverdue: 7, due: '18 May' },
];

const AGEING = [
  { range: '0–30 days', amount: '₹2,10,000', count: 4, pct: 37, color: '#2D7D46' },
  { range: '31–60 days', amount: '₹1,75,000', count: 2, pct: 31, color: '#D97706' },
  { range: '61–90 days', amount: '₹1,10,000', count: 1, pct: 20, color: '#EA580C' },
  { range: '90+ days', amount: '₹66,500', count: 1, pct: 12, color: '#DC2626' },
];
        ...
        <View style={s.tabs}>
          {(['outstanding', 'ageing'] as const).map(t => (
            <TouchableOpacity key={t} style={[s.tab, activeTab === t && s.tabActive]} onPress={() => setActiveTab(t)} activeOpacity={0.7}>
              <Text style={[s.tabTxt, activeTab === t && s.tabTxtActive]}>{t === 'outstanding' ? 'Outstanding' : 'Ageing Analysis'}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        {activeTab === 'outstanding' && (
          <View style={s.card}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 12 }} contentContainerStyle={{ gap: 8 }}>
              {['All', 'Overdue', 'Due This Week', 'Payments'].map(f => (
            ...
            {OUTSTANDING.map((p, i) => (
              <View key={p.id} style={[s.partyRow, i < OUTSTANDING.length - 1 && s.rowBorder]}>
                <View style={[s.avatar, { backgroundColor: '#DC2626' }]}>
                  <Text style={s.avatarTxt}>{p.name.charAt(0)}</Text>
                </View>
                <View style={s.partyInfo}>
                  <Text style={s.partyName}>{p.name}</Text>
                  <Text style={s.partyMeta}>{p.invoices} bill(s) · Due: {p.due}</Text>
                </View>
                <View style={s.partyRight}>
                  <Text style={[s.partyAmt, { color: '#DC2626' }]}>{p.amount}</Text>
                  {p.daysOverdue > 0 && <View style={s.overdueBadge}><Text style={s.overdueTxt}>{p.daysOverdue}d overdue</Text></View>}
                </View>
              </View>
            ))}
          </View>
            ...
            {AGEING.map(a => (
              <View key={a.range} style={s.ageRow}>
                <View style={s.ageLeft}>
                  ...
                  <Text style={s.ageCount}>{a.count} vendors</Text>
                </View>
                <View style={s.ageBar}>
