...
## [2026-07-06] R6 Party Form — Tally-fetched country + state masters (with fallback)

### Context
R4 delivered a multi-country dropdown (25 countries) but the state dropdown was still hardcoded to INDIAN_STATES. User reported: picking South Africa → state dropdown wrongly showed Punjab, Kerala, etc. Rather than hand-maintain 25 country-specific state lists (silent-mismatch risk against Tally masters), fetch directly from Tally's country + state master collections using new backend routes.

### Added
- **`src/services/api.ts`** — 2 new API wrappers:
  - `getTallyCountries(companyGuid?)` → `GET /api/tally/masters/countries`
  - `getTallyStates(companyGuid?, country?)` → `GET /api/tally/masters/states` (optional country filter)
- **`src/components/forms/PartyForm.tsx`:**
  - New optional prop `companyGuid?: string`.
  - `FALLBACK_COUNTRY_OPTIONS` + `FALLBACK_STATE_OPTIONS` consts (hardcoded 25 countries + 37 Indian states).
  - useEffect on `[companyGuid]` change — parallel `Promise.all` fetch of both endpoints, silent on error, sets state ONLY when response array is non-empty.
  - `countryOptions` prefers `tallyCountries` when populated, falls back to hardcoded.
  - `stateOptions` filters `tallyStates` by selected country (case-insensitive). Empty match falls back to `FALLBACK_STATE_OPTIONS` when country is India/empty; else empty array with helpful placeholder text.
  - Country + State dropdowns now consume these dynamic consts.
- **`app/ledger/create.tsx`:** passes `companyGuid={company?.guid}` to PartyForm.
- **`app/sales/create-invoice.tsx`:** passes `companyGuid={company?.guid}` to PartyForm.

### Not changed
- Existing GSTIN auto-detect via `stateFromGstin` (still uses hardcoded `GSTIN_STATE_CODE_MAP`) — UNCHANGED.
- Existing GST unlock logic (country + state + pincode required to reveal GSTIN section) — UNCHANGED.
- VAT toggle, address fields, all R1-R4 UI — UNCHANGED.
- Bank Details section (removed in R4) — still absent.
- Every write path (createLedger, createTallyParty) — UNCHANGED.

### Safety guarantees
- **India ledger flow untouched:** if Tally fetch fails/times-out/returns empty, hardcoded INDIAN_STATES falls back automatically. Same behavior as before R6.
- **Non-India flow now correct:** picking South Africa → state list from Tally (if desktop online + Tally accessible), else empty state list with placeholder "No states available for this country".
- **Silent failure:** no error toasts, no blocking spinners — fetch happens invisibly in background.
- **Zero coupling to R1-R5 backend XML wins:** this is dropdown UX only; write payloads unchanged.

### QA
🟢 GREEN — LITE subagent (14 checks all passed): TypeScript 0 errors, imports correct, fallback consts + useEffect + `Promise.all` wired, dropdown state derivation logic correct, GSTIN auto-detect untouched, parent callers pass companyGuid, R1-R4 regressions verified clean.
...
Requires backend R6 deployed + desktop R6 pulled + desktop app restarted for the live fetch to succeed. If any of those missing → falls back to hardcoded lists, no user-visible breakage. Test steps:
1. Restart desktop app.
2. Open Party Form on mobile.
3. Dropdown loads instantly (hardcoded fallback on first open, may take ~500ms to receive live fetch and switch).
4. Pick South Africa → state dropdown updates with Tally's SA provinces.
5. Pick India → state dropdown shows Indian states (from Tally or fallback — both should match).
6. Country dropdown may show more/fewer than 25 (depends on customer's Tally country master — e.g. some installs have 200+, some have 30).
